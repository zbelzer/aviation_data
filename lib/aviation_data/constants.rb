module AviationData
  ...
  HEADERS = {
    'MASTER'       => %w(identifier serial_number aircraft_model_code engine_mode_code year_manufactured type_registrant registrant_name address_1 address_2 registrant_city registrant_state registrant_zip registrant_region county_mail country_mail last_activity_date certificate_issue_date approved_operation_codes type_aircraft type_engine status_code transponder_code fractional_ownership airworthiness_date owner_one owner_two owner_three owner_four owner_five),
    'ACFTREF'      => %w(aircraft_model_code manufacturer model_name type_aircraft type_engine aircraft_category_code builder_certification_code engines seats weight cruising_speed),
    ...
    'AIRMEN'       => %w(unique_number first_name last_name address_1 address_2 city state zip country region medical_class medical_date medical_expiration_date),
    'CERTIFICATES' => %w(unique_number first_name last_name certificate_type level expiration_date) + RATINGS
  }
end
